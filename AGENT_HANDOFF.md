...
- **Frontend:** Vite + React + TypeScript SPA in `src/` — feature modules in `src/features/*` (dresses, reservations, payments, expenses, delivery-return, sales, reports, reminders, preferences, dashboard, device-lock, sync, observability, auth…), shared platform in `src/platform/*` (images, security, backups), app shell/router in `src/app/`, Supabase client in `src/lib/supabaseClient.ts` (env names only: §8).
...
- **Safety guards (client):** RM-1 cloud call timeout 15s (`LENA_CLOUD_TIMEOUT`) in `src/features/sync/showroomCloudState.ts`; M2 snapshot size cap 20 MiB client+server (`LENA_SNAPSHOT_TOO_LARGE`, 0016); M11 PIN throttle in `src/platform/security/devicePin.ts`.
