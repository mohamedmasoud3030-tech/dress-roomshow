...
| `src/platform/desktop/*` | Owns desktop snapshot bootstrap, mirror serialization/application, status events, retry attempts, 500ms synchronization, reload, and browser fallback; `src/services/desktopDatabase.ts` is a compatibility re-export only | `platform/desktop` | Existing four-scenario desktop behavior suite plus platform ownership test; compatible Tauri relaunch remains Phase 4 |
