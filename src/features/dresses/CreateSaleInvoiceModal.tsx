...
export function CreateSaleInvoiceModal({ open, onClose, onCreated }: Props) {
  ...
  return <Modal open={open} onClose={close} title="فاتورة مبيعات جديدة" className="max-w-3xl"><form onSubmit={submit} className="space-y-4">
    ...
    <div className="space-y-3"><div className="flex items-center justify-between"><h3 className="font-bold">بنود الفاتورة</h3><button type="button" onClick={() => setLines((current) => [...current, { dressCode: '', amount: '' }])} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-sm font-bold"><Plus className="h-4 w-4" />إضافة بند</button></div>{lines.map((line, index) => <div key={index} className="grid gap-3 rounded-xl border border-slate-200 p-3 md:grid-cols-[1fr_160px_44px]"><label className="text-sm font-bold text-slate-700">الفستان<select required value={line.dressCode} onChange={(event) => selectDress(index, event.target.value)} className={STACKED_FORM_FIELD_CLASS_NAME}><option value="">اختاري الفستان</option>{dresses.map((dress) => <option key={dress.id} value={dress.code}>{dress.code} — {dress.name}</option>)}</select></label><label className="text-sm font-bold text-slate-700">القيمة<input required type="number" min={MIN_MONEY_AMOUNT} step={MONEY_STEP} value={line.amount} onChange={(event) => updateLine(index, { amount: event.target.value })} className={STACKED_FORM_FIELD_CLASS_NAME} /></label><button type="button" aria-label="حذف البند" disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((_, lineIndex) => lineIndex !== index))} className="mt-6 flex h-11 items-center justify-center rounded-xl border border-rose-200 text-rose-700 disabled:opacity-40"><Trash2 className="h-4 w-4" /></button></div>)}</div>
