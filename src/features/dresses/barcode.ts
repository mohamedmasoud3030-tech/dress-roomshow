...
export function normalizeDressCodeIdentifier(code: string): string {
  return code.trim().toUpperCase().replace(/\s+/g, '');
}

export function encodeCode128B(value: string): string {
  const normalized = normalizeDressCodeIdentifier(value);
  if (!normalized) throw new Error('Dress code is required for barcode generation.');

  const codes = [...normalized].map((character) => {
    const valueCode = character.charCodeAt(0) - 32;
    if (valueCode < 0 || valueCode > 95) throw new Error('Dress code contains unsupported Code128 characters.');
    return valueCode;
  });

  const checksum = codes.reduce((total, code, index) => total + code * (index + 1), START_B) % 103;
  return [START_B, ...codes, checksum, STOP].map((code) => MODULE_PATTERNS[code]).join('');
}

export function barcodeModulesToSvg(modules: string, options: { height?: number; moduleWidth?: number } = {}): string {
  const height = options.height ?? 72;
  const moduleWidth = options.moduleWidth ?? 2;
  const width = modules.length * moduleWidth;
  const bars = [...modules].map((bit, index) => bit === '1' ? `<rect x="${index * moduleWidth}" y="0" width="${moduleWidth}" height="${height}" />` : '').join('');
