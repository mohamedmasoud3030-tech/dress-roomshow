...
function ReservationCard({ reservation, onCancel }: { reservation: Reservation; onCancel: (id: string) => void }) {
  const canCancel = reservation.status !== 'cancelled' && reservation.status !== 'returned';
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-2 xl:grid-cols-4">
        <div>
          <p className="text-xs font-bold text-slate-400">الاستلام</p>
          <p className="mt-1 text-sm font-semibold text-slate-800">{reservation.pickupDate}</p>
        </div>
        <div>
          <p className="text-xs font-bold text-slate-400">الإرجاع</p>
          <p className="mt-1 text-sm font-semibold text-slate-800">{reservation.returnDate}</p>
        </div>
        <div>
          <p className="text-xs font-bold text-slate-400">الإجمالي</p>
          <p className="mt-1 text-sm font-bold text-slate-950">{formatMoneyOMR(reservation.totalAmount)}</p>
        </div>
        <div>
          <p className="text-xs font-bold text-slate-400">المتبقي</p>
          <p className={`mt-1 text-sm font-bold ${reservation.remainingAmount > 0 ? 'text-rose-700' : 'text-emerald-700'}`}>
            {formatMoneyOMR(reservation.remainingAmount)}
          </p>
        </div>
      </div>
      ...
      {canCancel && (
        <div className="mt-4 flex justify-end border-t border-slate-100 pt-4">
          <button
            ...
            className="inline-flex min-h-10 items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-slate-500 transition hover:bg-rose-50 hover:text-rose-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:ring-offset-2"
          >
...
export function ReservationsPage() {
  ...
  const handleCreated = (reservation: Reservation) => {
    ...
    setFeedback({ tone: 'success', message: `تم إنشاء الحجز ${reservation.reservationNumber} بنجاح.` });
  };

  const handleCancel = (id: string) => {
    const reservation = reservations.find((item) => item.id === id);
    if (!reservation) return;
    if (!window.confirm(`هل تريدين إلغاء الحجز ${reservation.reservationNumber}؟`)) return;

    try {
      cancelReservation(id);
      setReservations(getReservations());
      setFeedback({ tone: 'success', message: `تم إلغاء الحجز ${reservation.reservationNumber}.` });
    } catch (error: unknown) {
      setFeedback({ tone: 'danger', message: error instanceof Error ? error.message : 'تعذر إلغاء الحجز.' });
    }
  };
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <PageHeader
          ...
          description="إنشاء الحجوزات، منع التعارضات، ومتابعة مواعيد الاستلام والإرجاع."
        />
        <button
          type="button"
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_190px_190px]">
          <label className="relative block">
            ...
            <input
              ...
              placeholder="ابحثي برقم الحجز أو العميلة أو الفستان"
      ...
      {filteredReservations.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
          {reservations.length === 0 ? (
            <>
              <CalendarCheck aria-hidden="true" className="mx-auto h-10 w-10 text-amber-700" />
              <p className="mt-4 text-lg font-bold text-slate-950">لا توجد حجوزات حتى الآن</p>
              <p className="mt-2 text-sm text-slate-500">ابدئي بإنشاء أول حجز وربطه بعميلة وفستان وفترة واضحة.</p>
              <button type="button" onClick={openCreateModal} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2">
            ...
            <>
              ...
              <p className="mt-4 text-lg font-bold text-slate-950">لا توجد حجوزات مطابقة</p>
