...
import { Ban, CheckCircle2, Plus, Search, Wrench } from 'lucide-react';
...
function TaskCard({ task, onComplete, onCancel }: { task: ServiceTask; onComplete: (task: ServiceTask) => void; onCancel: (task: ServiceTask) => void }) {
  ...
  return <article className={`rounded-2xl border bg-white p-5 shadow-sm ${overdue ? 'border-rose-200 ring-2 ring-rose-100' : 'border-slate-200'}`}>
    ...
    {activeStatuses.has(task.status) && <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => onComplete(task)} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-emerald-700 px-4 text-sm font-bold text-white"><CheckCircle2 className="h-4 w-4" />إكمال المهمة</button><button type="button" onClick={() => onCancel(task)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-rose-200 bg-white px-4 text-sm font-bold text-rose-700"><Ban className="h-4 w-4" />إلغاء المهمة</button></div>}
  </article>;
}
...
export function ServiceTasksPage() {
  ...
  function cancel(task: ServiceTask) {
    try {
      updateServiceTask(task.id, { status: 'cancelled' });
      setTasks(getServiceTasks());
      setFeedback({ tone: 'success', message: `تم إلغاء مهمة الخدمة ${task.taskNumber}.` });
    } catch (error: unknown) { setFeedback({ tone: 'danger', message: error instanceof Error ? error.message : 'تعذر إلغاء مهمة الخدمة.' }); }
  }
