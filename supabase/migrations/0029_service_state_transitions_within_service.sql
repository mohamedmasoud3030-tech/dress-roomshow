-- A piece already in service must still be able to enter another service state.
--
-- The transition matrix let a piece leave `maintenance` only for `available`,
-- `damaged` or `inactive`, and let a piece in `laundry` only reach `available`,
-- `maintenance`, `damaged` or `inactive`. So two ordinary follow-ups were
-- rejected with LENA_INVENTORY_TRANSITION_INVALID: a piece under maintenance
-- that also needs cleaning, and a piece back from the laundry that needs
-- another look. The only way out was to push the piece onto the shelf first,
-- which is exactly the shortcut the service queue exists to prevent.
--
-- This migration restates private.lena_assert_dress_transitions verbatim with
-- `inspection` and `laundry` reachable from every service state, and with
-- `inspection` reachable from `damaged` so a damaged piece can be assessed
-- before any repair is decided on.
